            ...
            <input className='searchtracks-input' placeholder='Find music!' onChange={inputHandler}></input>
             {
                items.filter(item => {
                 ...
                 <div key = { item.id } className='track-container' >
                    ...
                    <button class="btn btn-outline-dark" onClick={() => openInNewTab("https://www.youtube.com/results?search_query="+item.artistName+"-"+item.albumTitle+" "+item.trackTitle)}>Play Song</button></p>
                </div>
                ))
